...
class VotingService {
      ...
      if (status === OperationStatus.SpeculativeSuccess) {
        ...
        for (const event of events) {
          if (event.data.includes("Proposal created:")) {
            ...
            proposalId = Number.parseInt(eventParts[2]) // Extract the proposal ID
            break
          }
        }
  ...
  async getProposal(connectedAccount: any, proposalId: number): Promise<Proposal | null> {
    ...
    try {
      ...
      // Manual parsing without using expect
      ...
      try {
        const idValue = proposalArgs.nextU64()
        proposal.id = Number(idValue)

        const projectIdValue = proposalArgs.nextString()
        proposal.projectId = projectIdValue

        const titleValue = proposalArgs.nextString()
        proposal.title = titleValue

        const descriptionValue = proposalArgs.nextString()
        proposal.description = descriptionValue

        const creatorValue = proposalArgs.nextString()
        proposal.creator = creatorValue

        const startPeriodValue = proposalArgs.nextU64()
        proposal.startPeriod = Number(startPeriodValue)

        const endPeriodValue = proposalArgs.nextU64()
        proposal.endPeriod = Number(endPeriodValue)

        const statusValue = proposalArgs.nextU8()
        // Fix the type conversion issue by using Number() first
        proposal.status = Number(statusValue) as ProposalStatus

        const yesVotesValue = proposalArgs.nextU64()
        proposal.yesVotes = Number(yesVotesValue)

        const noVotesValue = proposalArgs.nextU64()
        proposal.noVotes = Number(noVotesValue)

        const totalVotesValue = proposalArgs.nextU64()
        proposal.totalVotes = Number(totalVotesValue)

        const executedValue = proposalArgs.nextBool()
        proposal.executed = executedValue
      } catch (err) {
  ...
  async getVotingPower(connectedAccount: any): Promise<number> {
    ...
    try {
      ...
      try {
        const powerValue = powerArgs.nextU64()
        votingPower = Number(powerValue)
      } catch (err) {
        console.error("Error parsing voting power:", err)
      }
  ...
  async getUserVote(connectedAccount: any, proposalId: number): Promise<VoteRecord | null> {
    ...
    try {
      ...
      try {
        const voterValue = voteArgs.nextString()
        voteRecord.voter = voterValue

        const proposalIdValue = voteArgs.nextU64()
        voteRecord.proposalId = Number(proposalIdValue)

        const supportValue = voteArgs.nextBool()
        voteRecord.support = supportValue

        const votingPowerValue = voteArgs.nextU64()
        voteRecord.votingPower = Number(votingPowerValue)
      } catch (err) {
