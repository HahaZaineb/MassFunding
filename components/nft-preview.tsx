...
export function NFTPreview({ projectId, amount, nftId, onBack, project }: NFTPreviewProps) {
  ...
  const viewOnExplorer = () => {
    toast({
      title: "Explorer View",
      description: `This would open the Massa Explorer to view NFT ${generatedNftId}`,
    })
  ...
  const downloadNFT = () => {
    ...
    if (!ctx) {
      toast({
        ...
        description: "Your browser doesn't support canvas operations",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      toast({
        title: "Download Failed",
        description: "Failed to generate NFT image. Please try again.",
        variant: "destructive",
      })
    }
  }
